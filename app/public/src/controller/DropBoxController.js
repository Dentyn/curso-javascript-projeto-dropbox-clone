...
class DropBoxController{
    constructor (){
        ...
        this.currentFolder = ['hcode']  //atributo para se localizar na pasta
        ...
        this.snackModalEl =  document.querySelector('#react-snackbar-root')//barra de progresso de upload
        ...
        this.btnNewFolfder = document.querySelector('#btn-new-folder')
    ...
    connectFirebase(){
        var firebaseConfig = {
            apiKey: "AIzaSyDKqCamGtfmdOK9ZpwBkWfbJx17yl1SAWI",
            authDomain: "drop-fbf78ss.firebaseapp.com",
            databaseURL: "https://drop-fbf78ss.firebaseio.com",
            projectId: "drop-fbf78ss",
            storageBucket: "drop-fbf78ss.appspot.com",
            messagingSenderId: "506412373563",
            appId: "1:506412373563:web:38fea68d8a11c552a4ae1e",
            measurementId: "G-6WZY4D4NM0"
          };
    ...
    removeFolderTask(ref,name){//esse metodo vale para pastas que tem filhos

        return new Promise((resolve,reject)=>{
            let folderRef =this.getFirebaseRef(ref + '/' + name)     //buscando o caminho da pasta
    ...
    removeTask(){
        let promises = []
        //busca os arquivos que foram selecionados para a exclusao
        this.getSelection().forEach(li =>{//pega os arquivos foram selecionados na tela

            let file = JSON.parse(li.dataset.file)//pegando os dados(que foram selecionados) e convertendo um parse
            let key = li.dataset.key
            
            let formData =  new FormData()//dados que quero mandar para o servidor
            

            /****************passou a ser usado o banco de dados do firebase*******************/
            // formData.append('path'/*nome do campo que vc quer enviar*/,file.path)
            // formData.append('key'/*nome do campo que vc quer enviar*/,key)

            //promises.push(this.ajax('/file','DELETE',formData/*dados que qero mandar*/)) // solicitacao de http 
    ...
    removeFile(ref,name){
        //referencia do arquivo que estou querendo excluir
    ...
    initEvents(){//esse metodo lida com a selecao de arquivos. Nao podemos renomear um sequencia de arquivos.Esse metodo lida com esse tipo de problemas
        
        this.btnNewFolfder.addEventListener('click', event =>{
            let nomedapasta = prompt('Nome da nova pasta:')
            
            if (nomedapasta) {
                this.getFirebaseRef().push()/*Dentro da minha pasta(hcode)*/.set({//adcione essa pasta
                    name : nomedapasta,
                    type: 'folder',//faz licgacao com swtch adcionando o icone de pasta
                    path:this.currentFolder.join('/')//caminho da pasta,o join eh pra construir o caminho da pasta 
                })
            }
        })
        ...
        this.btnDelete.addEventListener('click', e =>{
            ...
            this.removeTask().then/*recebendo a promessa*/(responses =>{//promise
                ...
                responses.forEach(response=>{
                    ...
                    if(response.fields.key){
                        ...
                        this.getFirebaseRef().child(response.fields.key).remove()//removendo do firebase 
                    }
        ...
        this.btnRename.addEventListener('click', e =>{
            ...
            let file = JSON.parse(li.dataset.file)//buscando a informacao que sera renomeada
        ...
        this.listFilesEl.addEventListener('selectionchange', e=>{
            ...
            switch(this.getSelection().length/*retorna a qte de elementos selecionados*/){

                case 0:
                    this.btnDelete.style.display = 'none'//deixa todos os botoes on
        ...
        this.inputFilesEl.addEventListener('change' , event=>{
            console.log(event.target/*qual alvo o evento foi disparado elemento do input*/.files)
            
            this.btnSendFileEl.disable = true

            this.uploadTask(event.target.files).then(responses =>{//fica responsavel por receber multiplos arquivos
    ...
    getFirebaseRef(path){
        ...
        return firebase.database()/*utilizando o servico de data base*/.ref(path/*'files'->buscando a referencia*/)//salvando informacao do arquivo,ou seja, o arquivo em si nao foi enviado
    ...
    ajax(url, method = 'GET',formData = new FormData(),onprogress = function(){},onloadstart = function(){}){
        ...
        return new Promise((resolve,reject)=>{//retorna um promisse pq pode da' errado 
            
            
            let ajax = new XMLHttpRequest()//criacao de um nova instancia
                
            
            ajax.open(method ,url)//envia para uma pasta chamada upload
                ...
                ajax.onerror = event =>{
                    ...
                    reject(event)//passando a informacao do erro
                }

                ajax.upload.onprogress = onprogress;
                onloadstart()//momento da hora nao qual foi iniciado  upload
                ajax.send(formData);//envia as informcao via ajax,necessariamente o envio do arquivo

                
                
                
                //let formData = new FormData()
                //formData.append('input-file'/*nome do campo no qual o post recebe (servidor)*/,file/*qual o arquivo esta sendo enviado de fato(forEach)*/)
    ...
    uploadTask(files){//metodo responsavel por receber os arquivos
        
        
        
        let promises/*promises pq cada arquivo pode acontecer o uploado oo poode falhar */ = [];//cada arquivo vai ser uma promessa
        [...files].forEach(file /*Para cada arquivo que esta sendo recebido*/ =>{//execute essa funcao
            ...
            //     this.startUploadTime = Date.now()//registro da hora 
            
            // }))

            /*************************Salvando no servidor do firebase***************************/

           
            promises.push(new Promise((resolve,reject)=>{

                let fileRef = firebase.storage().ref(this.currentFolder.join('/')).child(file.name)//fazendo apenas o registro.Nao foi feito o upload do arquivo ainda
           
                let task = fileRef.put(file)


                task.on('state_changed',/*progress*/snapshot/*fotografia atual do arquivo,ou seja, como esse arquivo esta no firebase*/=>{
                    this.uploadProgress({
        ...
        return Promise.all(promises)//recebe um array e promessa
    }
    ...
    getFileIconView(file){//metodo para retorna apenas os icones, foi extraidos trecho de codigos do arquivo httml original deixando mais limpo
    ...
    readFile(){
        this.lastFolder = this.currentFolder.join('/')//armazenado a ultima pasta,ou seja, a atual pasta
        this.getFirebaseRef().on('value',snapshot =>{/*Resposnsavel por informar mudancas no servidor*/

            this.listFilesEl.innerHTML = ''//limpa 

            snapshot.forEach(snapshotItem /*cadfa um dos items que estao ali dentro*/=>{
                let key = snapshotItem.key
                let data = snapshotItem.val()
                
                
                if(data.type){//lida com arquivo que nao tem necessidade de ser renderizadoi
    ...
    openFolder(){
        ...
        if(this.lastFolder) this.getFirebaseRef/*buscando a referencia no firebase*/(this.lastFolder).off('value')//desliga a ultima pasta/evento (escuta) 
    ...
    renderNav(){//metodo responsavel por situar onde o usuario se encontra na pasta de navegacao
        
        let nav = document.createElement('nav')
        let path = [] //salvando o caminho de cada uma das pastas
        ...
        for(let i = 0; i < this.currentFolder.length;i++ ){
            ...
            if ((i +1) === this.currentFolder.length)/*busca a ultima posicao do array,pq o layout de escrita eh diferente */ {
                span.innerHTML = folderName
            }else{

                //conteudo a ser adiciona do html
            ...
            nav.appendChild/*so aceita "node"*/(span)
    ...
    initEventLi(li){
        ...
        //criando um evento de duplo click
        ...
        li.addEventListener('click', e =>{
                ...
                if(firstLi){
                    ...
                    lis.forEach((el, i)=>{
                        ...
                        if(i >= index[0] && i <= index[1]){
                            el.classList.add('selected')//adcao de uma class no li, apenas no que foi clicado
                        }
            ...
            if(!e.ctrlKey){
                ...
                this.listFilesEl.querySelectorAll('li.selected')/*procura por todos li selecionados(class)*/.forEach(el =>{
                    el.classList.remove('selected')//a parti do li selecionados remove a class
                })
            }
